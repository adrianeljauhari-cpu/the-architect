...
- Posiciones: p1 = `oferta pos_ofer=0` (proveedor, por segmento); **p2 = `oferta_cli`** (por tipo/grupo
  o por cliente); p3 = `oferta pos_ofer=1` (droguería, por segmento); p4 = `customers.desc_glob`.
  `descuen` está **vacía** — no hay escalas de volumen; la posición 2 la alimenta `oferta_cli`.
- **Las ofertas por segmento se aplican por el SEGMENTO del cliente**, no por texto: aplican cuando
...
- `oferta_cli` (posición 2) aplica por tipo/grupo (`tipo_d/h`) o por cliente (`co_cli_d/h`) y **se
  encadena multiplicativamente** con las demás posiciones (confirmado).
